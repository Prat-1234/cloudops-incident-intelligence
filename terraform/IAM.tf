...
resource "aws_iam_role" "lambda_ingestion" {
  name = "${var.project}-lambda-ingestion-role"
...
resource "aws_iam_role_policy" "lambda_ingestion_policy" {
  name = "${var.project}-ingestion-policy"
...
resource "aws_iam_role" "lambda_guardduty" {
  name = "${var.project}-lambda-guardduty-role"
...
resource "aws_iam_role_policy" "lambda_guardduty_policy" {
  name = "${var.project}-guardduty-policy"
...
resource "aws_iam_role" "ec2_log_generator" {
  name = "${var.project}-ec2-log-generator-role"
...
resource "aws_iam_role_policy" "ec2_log_generator_policy" {
  name = "${var.project}-ec2-log-policy"
  role = aws_iam_role.ec2_log_generator.id
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "arn:aws:logs:${var.aws_region}:${data.aws_caller_identity.current.account_id}:log-group:/cloudops/app-logs:*"
    }]
  })
}

resource "aws_iam_instance_profile" "ec2_log_generator" {
  name = "${var.project}-ec2-instance-profile"
  role = aws_iam_role.ec2_log_generator.name
}